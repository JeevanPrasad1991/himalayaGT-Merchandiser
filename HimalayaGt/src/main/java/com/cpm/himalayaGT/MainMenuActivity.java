                ...
                if (jcplist.size() == 0) {
                    ...
                    if (cdata.size() > 0) {
                        for (int i = 0; i < cdata.size(); i++) {
                            if (cdata.get(i).getStatus().equalsIgnoreCase(CommonString1.KEY_VALID) || cdata.get(i).getStatus().equalsIgnoreCase(CommonString1.KEY_INVALID)) {
                        ...
                        public void onClick(DialogInterface dialog, int id) {
                            try {
                                ...
                                if (sd.canWrite()) {
                                    ...
                                    String currentDBPath = "//data//com.cpm.himalayaGT//databases//" + GSKDatabase.DATABASE_NAME;

                                    String backupDBPath = "HimalayaGT_backup" + dateString.replace('/', '-');
